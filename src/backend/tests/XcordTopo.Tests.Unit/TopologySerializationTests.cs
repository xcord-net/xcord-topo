...
public class TopologySerializationTests : IDisposable
{
    ...
    [Fact]
    public void Hcl_Variables_IncludeSshPrivateKey()
    {
        // Users provide SSH keys for instance access — the private key must
        // be declared as a variable so Terraform can use it in connection blocks.
        var files = GenerateMultiProviderHcl();
        var variables = files["variables.tf"];

        Assert.Contains("ssh_private_key", variables);
    }
    ...
    public void Hcl_LiveKitKeys_NoSpaceInValue()
    {
        ...
        Assert.DoesNotContain(".result}: ${random_password", liveKitSection);
        // Verify the format is key:secret with no space
        Assert.Contains(".result}:${random_password", liveKitSection);
    }
